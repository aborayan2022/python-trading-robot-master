"""Crypto paper daily session — PaperBroker pipeline for cryptocurrencies.

This session runs ``CryptoTrendBreakoutStrategy`` against the ``CryptoProvider``
(yfinance) data through the standard ``build_default_pipeline`` (PaperBroker).

Crypto markets trade 24/7/365 — there is no market close, no holidays, and
no after-hours period.  Daily bars are aggregated over UTC midnight boundaries.
...
Modes:
    ...
    (default)     auto (always proceeds for daily crypto bars)

Environment:
    PYROBOT_DATA_DIR    root for data/ (default ".")
    PYROBOT_UNIVERSE    comma-separated crypto symbols (default BTC-USD,ETH-USD)

Run:
    .venv/bin/python crypto_paper_session.py --smoke
    .venv/bin/python crypto_paper_session.py --now
    .venv/bin/python crypto_paper_session.py --dry-run
"""
...
from pyrobot.data.crypto_provider import DEFAULT_CRYPTO, TRADING_CALENDAR, CryptoProvider
...
def _save_report(payload: Dict[str, Any], root: Path) -> Path:
    out = _report_dir(root) / f"crypto_paper_session_{datetime.now(timezone.utc):%Y%m%d_%H%M%S}.json"
    out.write_text(json.dumps(payload, indent=2, default=str), encoding="utf-8")
    return out

def _session_log_path(root: Path) -> Path:
    return _report_dir(root) / f"crypto_paper_session_{_session_date()}.jsonl"
...
def _seed_from_cache(root: Path, symbol: str, seed_bars: int) -> Dict[str, Any]:
    """Load seed bars from the yfinance CSV cache (no network)."""
    cache_path = root / "data" / "crypto" / f"{symbol.replace('-', '_')}.csv"
...
def _fetch_current_daily_bar(root: Path, symbol: str) -> Optional[Dict[str, Any]]:
    """Fetch today's (or most recent) daily bar — yfinance cache only."""
    cache_path = root / "data" / "crypto" / f"{symbol.replace('-', '_')}.csv"
    if not cache_path.exists():
        # Download minimal 30-day history via provider
        provider = CryptoProvider(symbols=[symbol], data_dir=root / "data" / "crypto")
...
def run_smoke(opts: Dict[str, Any], report: Dict[str, Any]) -> Dict[str, Any]:
    ...
    print("=" * 72)
    print("PyRobot — Crypto Paper Session SMOKE (cached data + pipeline wiring)")
    print("=" * 72)
    ...
    for sym in symbols:
        entry = _seed_from_cache(opts["data_dir"], sym, opts["seed_bars"])
    ...
    strategy = CryptoTrendBreakoutStrategy(strategy_id="crypto_trend", symbols=symbols)
...
def run_session(opts: Dict[str, Any], report: Dict[str, Any], *, dry_run: bool = False) -> Dict[str, Any]:
    ...
    symbols = opts["symbols"]
    root = opts["data_dir"]
    audit_path = str(_audit_dir(root) / f"crypto_paper_session_{_session_date()}.jsonl")
    ...
    state_path = root / "data" / "paper_state" / "crypto_paper.json"
    ...
    for sym in symbols:
        ...
        entry = _seed_from_cache(root, sym, opts["seed_bars"])
        seeded[sym] = {"source": entry["source"], "bars": len(entry["bars"])}
        seed_entries[sym] = entry["bars"]
    ...
    missing: List[str] = []
    for sym in symbols:
        b = _fetch_current_daily_bar(root, sym)
        if b is not None:
            bars[sym] = b
        else:
            missing.append(sym)
...
def main() -> None:
    report: Dict[str, Any] = {
        "title": "Crypto Paper Session — CryptoTrendBreakoutStrategy (PaperBroker)",
