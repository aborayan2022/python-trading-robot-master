...
> **Follow-up re-run (2026-09-11):** the interim metals figure (−17.58% / 275 trades) shipped in commit `a542855` was produced from an intermediate working-tree state (runner short-side / accounting edits still in flight) and does **not** reproduce against the committed code. All six strategy backtests were re-run in full from committed HEAD, then re-run **again from the clean, provenance-certified commit `522f98e`** (files `data/reports/*_backtest_20260911_07xxxx.json`, each carrying `provenance.git_commit` + `dirty_tree=false`); the numbers in this section reflect those runs, and the reviewer's own clean-HEAD reproduction matched them figure-for-figure. Net effect: `metals_trend` degrades to −57.5% and `metals_momentum` improves to −12.0%; nothing crosses the buy-and-hold bar, and the short-side conclusion is unchanged but stronger.
