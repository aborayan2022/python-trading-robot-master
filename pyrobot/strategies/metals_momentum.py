...
class MetalsMomentumBreakout(MultiSymbolStrategy):
    ...
    def sync_positions(self, positions: Dict[str, float]) -> None:
        ...
        for symbol in self._symbols:
            ...
            logger.info(
                "MetalsMomentumBreakout %s synced position for %s: qty=%s holding=%s direction=%s",
            ...
            if self._holding_direction.get(symbol) == "long":
                ...
                if close < self._stop_level[symbol]:
                    return self._make_signal(symbol, SignalAction.SELL, 0.85,
                        f"ATR trailing stop: close {close:.2f} < stop {self._stop_level[symbol]:.2f} (from high {self._extreme_high[symbol]:.2f}, entry {entry_p:.2f})")
            else:
                ...
                if close > self._stop_level[symbol]:
                    return self._make_signal(symbol, SignalAction.BUY_TO_COVER, 0.85,
                        f"ATR trailing stop: close {close:.2f} > stop {self._stop_level[symbol]:.2f} (from low {self._extreme_low[symbol]:.2f}, entry {entry_p:.2f})")
