...
from pyrobot.audit.ledger import AuditLedger
...
def main() -> None:
    ...
    logger.info(
        "Starting runtime loop: symbols=%s bars=%d mode=%s source=%s balance=%.0f audit=%s",
        symbols, n_bars, mode, type(source).__name__, balance, audit_path,
    )
    loop = TradingLoop(
        pipeline=pipeline,
        bar_provider=replay_provider(generate_replay_data(symbols, n_bars, seed=seed)),
