"""Cryptocurrency data provider via yfinance.

Downloads and caches daily OHLCV for major cryptocurrencies using Yahoo Finance
tickers (BTC-USD, ETH-USD). Data is cached in ``data/crypto/`` for offline
backtesting.
...
Environment:
    ...
    PYROBOT_CRYPTO_YEARS  years of history (default 5)
"""
...
logger = get_logger("crypto_provider")

ROOT = Path(os.environ.get("PYROBOT_DATA_DIR", ".")).resolve()
DATA_DIR = ROOT / "data" / "crypto"

# Default crypto universe — major assets.
DEFAULT_CRYPTO = ["BTC-USD", "ETH-USD"]

# Trading calendar: 24/7/365
TRADING_CALENDAR = {
    "name": "Crypto_24_7",
    "hours": "24 hours / 7 days / 365 days",
    "closed": "Never — no market close, no holidays",
    "timezone": "UTC",
}


class CryptoProvider(MarketDataProvider):
    """yfinance-based provider for cryptocurrency data.

    Caches downloaded data as CSV in ``data/crypto/`` for reproducible
    ...
    def __init__(
        ...
        years: int = int(os.environ.get("PYROBOT_CRYPTO_YEARS", "5")),
        data_dir: Optional[Path] = None,
    ) -> None:
        self._symbols = list(symbols or DEFAULT_CRYPTO)
        self._years = years
    ...
    def _download(self, symbol: str) -> Optional[pd.DataFrame]:
        try:
            ...
            logger.error("yfinance is required for CryptoProvider: pip install yfinance")
            return None
    ...
    def download_all(self, refresh: bool = False) -> Dict[str, pd.DataFrame]:
        """Download/cache all default crypto symbols. Returns symbol → DataFrame."""
    ...
    def load_cached(self) -> Dict[str, pd.DataFrame]:
        """Load all cached crypto data without downloading."""
        frames: Dict[str, pd.DataFrame] = {}
        for symbol in self._symbols:
            df = self._load_or_download(symbol)
            if df is not None and not df.empty:
