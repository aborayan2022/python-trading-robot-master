...
def calculate_quantitative_metrics(
    ...
    sharpe = float(np.sqrt(252) * excess_returns.mean() / returns.std()) if returns.std() > 0 else 0.0

    downside = returns[returns < 0]
    sortino = float(np.sqrt(252) * excess_returns.mean() / downside.std()) if len(downside) > 0 and downside.std() > 0 else 0.0
